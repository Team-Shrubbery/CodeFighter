...
class Player2(pygame.sprite.Sprite):
    def __init__(self, game):
        self.game = game
        self.image = self.game.fixer_sprite_sheet.get_sprite(5, 4, 116, 104)
...
# -------------- Movement --------------
        ...
        self.running = False
        self.move_frame = 0
        self.run_ani_R = [self.game.fixer_sprite_sheet.get_sprite(268, 795, self.width, self.height), self.game.fixer_sprite_sheet.get_sprite(75, 793, self.width, self.height),
        self.game.fixer_sprite_sheet.get_sprite(54, 927, self.width, self.height)]

        self.run_ani_L = [self.game.fixer_sprite_sheet.get_sprite(268, 795, self.width, self.height), self.game.fixer_sprite_sheet.get_sprite(75, 793, self.width, self.height),
        self.game.fixer_sprite_sheet.get_sprite(54, 927, self.width, self.height)]
    ...
    def basic_health(self): # draws the health bar inside a box
        if self.cur_health <= (MAX_HEALTH / 2):
            if self.cur_health <= (MAX_HEALTH / 4):
                pygame.draw.rect(self.game.screen, RED, (350, 10, self.cur_health/self.health_ratio, 25))
            else:
                pygame.draw.rect(self.game.screen, YELLOW, (350,10, self.cur_health/self.health_ratio, 25))
        else:
            pygame.draw.rect(self.game.screen, GREEN, (350, 10, self.cur_health/self.health_ratio, 25))
        pygame.draw.rect(self.game.screen, WHITE, (350, 10, self.healthbar_length, 25), 2)
